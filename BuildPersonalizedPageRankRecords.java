...
public class BuildPersonalizedPageRankRecords extends Configured implements Tool {
	...
	private static class MyMapper extends Mapper<LongWritable, Text, IntWritable, PageRankNode> {
		...
		public void setup(Mapper<LongWritable, Text, IntWritable, PageRankNode>.Context context) {
			...
			if (n == 0 || srcNodeArray == null) {
				throw new RuntimeException(
						NODE_CNT_FIELD + " cannot be 0!" + "or" + SRC_NODE_ARRAY_FIELD + "cannot be null");
			}
		...
		public void map(LongWritable key, Text t, Context context) throws IOException, InterruptedException {
			...
			if (arr.length == 1) {
				...
				// node.setPageRankList(new ArrayListOfFloatsWritable());
			...
			// Setting page rank for sources as 0
			String currentNode = arr[0].trim();
			float[] pRValues = new float[srcNodeArray.length];
			for (int i = 0; i < srcNodeArray.length; i++) {
				String srcVal = srcNodeArray[i];
				if (srcVal.equalsIgnoreCase(currentNode)) {
					pRValues[i] = 0f;
				} else {
					pRValues[i] = Float.NEGATIVE_INFINITY;
				}
			}
			node.setPageRankList(new ArrayListOfFloatsWritable(pRValues));
	...
	public int run(String[] args) throws Exception {
		...
		if (null != sourceNodeValues && sourceNodeValues.length > 0) {
			conf.setStrings(SRC_NODE_ARRAY_FIELD, sourceNodeValues);
		}
