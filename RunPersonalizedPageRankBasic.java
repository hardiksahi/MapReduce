...
public class RunPersonalizedPageRankBasic extends Configured implements Tool {
	...
	private static final String SRC_NODE_ARRAY_FIELD = "srcnode.array";
	private static final String MISSING_MASS = "MISSING_MASS";
	...
	private static class MapClass extends Mapper<IntWritable, PageRankNode, IntWritable, PageRankNode> {
		...
		private static String[] srcNodeArray = null;
		private static int[] srcArrayInt = null;

		@Override
		public void setup(Mapper<IntWritable, PageRankNode, IntWritable, PageRankNode>.Context context) {
			srcNodeArray = context.getConfiguration().getStrings(SRC_NODE_ARRAY_FIELD, null);
			if (srcNodeArray == null) {
				throw new RuntimeException(SRC_NODE_ARRAY_FIELD + "cannot be null");
			}

			System.out.println("srcNodeArray" + srcNodeArray);
			System.out.println("+ srcNodeArray.length " + srcNodeArray.length);
			srcArrayInt = new int[srcNodeArray.length];
			for (int i = 0; i < srcNodeArray.length; i++) {
				srcArrayInt[i] = Integer.parseInt(srcNodeArray[i]);
			}
		}
		...
		public void map(IntWritable nid, PageRankNode node, Context context) throws IOException, InterruptedException {
			...
			if (node.getAdjacenyList().size() > 0) {
				...
				// Array of mass to be distributed for each of the sources.
				float[] massArray = new float[srcArrayInt.length];

				for (int i = 0; i < srcArrayInt.length; i++) {
					float srcPr = node.getPageRankList().get(i);
					massArray[i] = srcPr - (float) StrictMath.log(list.size());
					// if (srcPr == Float.NEGATIVE_INFINITY) {
					// massArray[i] = srcPr;
					// } else {
					// massArray[i] = srcPr - (float) StrictMath.log(list.size());
					// }
				...
				for (int i = 0; i < list.size(); i++) {
					...
					intermediateMass.setPageRankList(new ArrayListOfFloatsWritable(massArray));
	...
	private static class CombineClass extends Reducer<IntWritable, PageRankNode, IntWritable, PageRankNode> {
		...
		public void reduce(IntWritable nid, Iterable<PageRankNode> values, Context context)
			...
			float[] prArray = new float[srcArrayInt.length];
			Arrays.fill(prArray, Float.NEGATIVE_INFINITY);
			...
			for (PageRankNode n : values) {
				if (n.getType() == PageRankNode.Type.Structure) {
					...
					for (int i = 0; i < srcArrayInt.length; i++) {
						prArray[i] = sumLogProbs(prArray[i], n.getPageRankList().get(i));
					}
			...
			if (massMessages > 0) {
				...
				intermediateMass.setType(PageRankNode.Type.Mass);
				intermediateMass.setPageRankList(new ArrayListOfFloatsWritable(prArray));
	...
	private static class ReduceClass extends Reducer<IntWritable, PageRankNode, IntWritable, PageRankNode> {
		// For keeping track of PageRank mass encountered, so we can compute missing
		// PageRank mass lost
		// through dangling nodes.
		private float[] totalMassArray = null;
		...
		public void setup(Reducer<IntWritable, PageRankNode, IntWritable, PageRankNode>.Context context) {
			...
			totalMassArray = new float[srcArrayInt.length];
			Arrays.fill(totalMassArray, Float.NEGATIVE_INFINITY);
		}
		...
		public void reduce(IntWritable nid, Iterable<PageRankNode> iterable, Context context)
			...
			// Create the node structure that we're going to assemble back together from
			// shuffled pieces.
			...
			// float mass = Float.NEGATIVE_INFINITY;
			float[] massArray = new float[srcArrayInt.length];
			Arrays.fill(massArray, Float.NEGATIVE_INFINITY);
			...
			while (values.hasNext()) {
				...
				if (n.getType().equals(PageRankNode.Type.Structure)) {
					...
					for (int i = 0; i < srcArrayInt.length; i++) {
						massArray[i] = sumLogProbs(massArray[i], n.getPageRankList().get(i));
					}
					// mass = sumLogProbs(mass, n.getPageRank());
			...
			node.setPageRankList(new ArrayListOfFloatsWritable(massArray));
			...
			if (structureReceived == 1) {
				...
				for (int i = 0; i < srcArrayInt.length; i++) {
					totalMassArray[i] = sumLogProbs(totalMassArray[i], massArray[i]);
				}
				// totalMass = sumLogProbs(totalMass, mass);
			} else if (structureReceived == 0) {
				// We get into this situation if there exists an edge pointing to a node which
				// has no
				...
				// It's important to note that we don't add the PageRank mass to total... if
				// PageRank mass
		...
		public void cleanup(Context context) throws IOException {
			...
			byte[] byteArray = floatToByteArray(totalMassArray);
			out.write(byteArray, 0, byteArray.length);// TODO
			out.close();
		}

		public static byte[] floatToByteArray(float[] values) {
	...
	private static class MapPageRankMassDistributionClass
		...
		private static String[] srcNodeArray = null;
		private static String[] missingMassArray = null;
		private static int[] srcIntArray = null;
		private static float[] missMassFloatArray = null;

		@Override
		public void setup(Context context) throws IOException {
			Configuration conf = context.getConfiguration();
			srcNodeArray = conf.getStrings(SRC_NODE_ARRAY_FIELD, null);
			missingMassArray = conf.getStrings(MISSING_MASS, null);
			if (srcNodeArray == null || missingMassArray == null || srcNodeArray.length != missingMassArray.length) {
				throw new RuntimeException(
						SRC_NODE_ARRAY_FIELD + " or " + MISSING_MASS + "cannot be null or length of arrays not same");
			}

			srcIntArray = new int[srcNodeArray.length];
			for (int i = 0; i < srcNodeArray.length; i++) {
				srcIntArray[i] = Integer.parseInt(srcNodeArray[i]);
			}

			missMassFloatArray = new float[srcNodeArray.length];
			for (int i = 0; i < srcNodeArray.length; i++) {
				missMassFloatArray[i] = Float.parseFloat(missingMassArray[i]);
			}
		...
		public void map(IntWritable nid, PageRankNode node, Context context) throws IOException, InterruptedException {
			ArrayListOfFloatsWritable prList = node.getPageRankList();
			int currentNode = nid.get();
			float p = 0f;
			for (int i = 0; i < srcIntArray.length; i++) {
				int srcVal = srcIntArray[i];
				if (srcVal == currentNode) {
					float jump = (float) (Math.log(ALPHA));
					float link = (float) Math.log(1.0f - ALPHA)
							+ sumLogProbs(prList.get(i), (float) (Math.log(missMassFloatArray[i])));

					p = sumLogProbs(jump, link);
				} else {
					p = (float) StrictMath.log(1.0f - ALPHA) + prList.get(i);
				}
				node.getPageRankList().set(i, p);
			}
			context.write(nid, node);
		}
	}
	...
	public int run(String[] args) throws Exception {
		...
		// Integer[] sourceNodeInt = new Integer[sourceNodeValues.length];

		// for (int i = 0; i < sourceNodeValues.length; i++) {
		// sourceNodeInt[i] = Integer.parseInt(sourceNodeValues[i]);
		// }
		// Iterate PageRank.
		for (int i = s; i < e; i++) {
			iteratePageRank(i, i + 1, basePath, n, useCombiner, useInmapCombiner, sourceNodeValues);
		}
	...
	private void iteratePageRank(int i, int j, String basePath, int numNodes, boolean useCombiner,
			boolean useInMapperCombiner, String[] sourceNodeValues) throws Exception {
		// Each iteration consists of two phases (two MapReduce jobs).

		// Job 1: distribute PageRank mass along outgoing edges.
		float[] massArray = phase1(i, j, basePath, numNodes, useCombiner, useInMapperCombiner, sourceNodeValues);
		System.out.println("Mass array is ::");
		for (float f : massArray) {
			System.out.println(f);
		}

		// Find out how much PageRank mass got lost at the dangling nodes.
		float[] missingArray = new float[massArray.length];
		for (int ind = 0; ind < massArray.length; ind++) {
			missingArray[ind] = 1.0f - (float) StrictMath.exp(massArray[ind]);
			if (missingArray[ind] < 0) {
				missingArray[ind] = 0;
			}
		}

		System.out.println("Missing mass array is ::");
		for (float f : missingArray) {
			System.out.println(f);
		}
		// float missing = 1.0f - (float) StrictMath.exp(mass);

		// Job 2: distribute missing mass, take care of random jump factor.
		phase2(i, j, missingArray, basePath, numNodes, sourceNodeValues);
	}
	...
	private float[] phase1(int i, int j, String basePath, int numNodes, boolean useCombiner,
		...
		if (null != sourceNodeValues && sourceNodeValues.length > 0) {
			job.getConfiguration().setStrings(SRC_NODE_ARRAY_FIELD, sourceNodeValues);
		}
		...
		// float massArray = Float.NEGATIVE_INFINITY;
		float[] massArray = new float[sourceNodeValues.length];
		Arrays.fill(massArray, Float.NEGATIVE_INFINITY);

		FileSystem fs = FileSystem.get(getConf());
		for (FileStatus f : fs.listStatus(new Path(outm))) {
			FSDataInputStream fin = fs.open(f.getPath());
			int nBytesToRead = 4 * massArray.length;
			byte[] byteArray = new byte[nBytesToRead];
			int bytesRead = fin.read(byteArray); // reading the byte array from the file system...

			// Convert byteArray to float Array:
			float[] corresFloatArray = byte2FloatArray(byteArray);

			for (int ind = 0; ind < massArray.length; ind++) {
				massArray[ind] = sumLogProbs(massArray[ind], corresFloatArray[ind]);
			}
	...
	public static float[] byte2FloatArray(byte[] values) throws IOException {
		ByteArrayInputStream bas = new ByteArrayInputStream(values);
		DataInputStream ds = new DataInputStream(bas);
		float[] fArr = new float[values.length / 4]; // 4 bytes per float
	...
	private void phase2(int i, int j, float[] missingArray, String basePath, int numNodes, String[] sourceNodeValues)
			throws Exception {
		Job job = Job.getInstance(getConf());
		...
		String[] missingMassString = new String[missingArray.length];
		for (int k = 0; k < missingArray.length; k++) {
			missingMassString[k] = Float.toString(missingArray[k]);
		}
		job.getConfiguration().setStrings(MISSING_MASS, missingMassString);
		job.getConfiguration().setStrings(SRC_NODE_ARRAY_FIELD, sourceNodeValues);
